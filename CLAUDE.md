...
11. **第一次要把某個素材檔上傳到 ElevenLabs 轉錄之前，先取得客戶同意**：用白話講清楚「我要把《某某檔案》上傳到 ElevenLabs（一個雲端的語音轉文字服務）幫你轉成文字，會用掉你金鑰的免費額度」，客戶說好才上傳。同一個檔案轉錄過就重用快取結果，不用重複問；但換一個新檔案（例如新的訪談影片）就要再問一次。這條適用所有轉錄場景（剪片的 A-ROLL、`reels-transcribe` 下載的影片都算）——客戶的素材可能含隱私內容，上傳雲端前他有權知道並說不。

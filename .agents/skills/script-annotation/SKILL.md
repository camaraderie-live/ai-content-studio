...
使用者常會直接貼一整段旁白，每句後面用括號註明想要的畫面。完整格式、給使用者看的「動畫點菜單」與範例在 [腳本動畫標註.md](../../../腳本動畫標註.md)；本 skill 是你（Codex）內部執行時要遵守的規則。全程講**繁體中文**。
...
1. **先複述，再動手**（video-use SKILL.md Hard Rule 11 + 專案規則檔中的「複述確認」原則，例如根目錄的 AGENTS.md / GEMINI.md）：逐條講一次「這句話我會怎麼呈現」，把每一條的做法、缺的素材、模糊的地方講清楚，使用者說「對」才開始剪。標註常常很精簡，你以為的和他想的可能不一樣。
2. **缺素材就列清單請他補**：對照下表「需要素材」的項目（Codex 動畫、demo 影片、截圖、要飛入的圖等），請他用數字開頭命名丟進專案的 `B-ROLL/`。**不要自己腦補替代品硬做。**
...
5. **Reels／Stories 安全區不可違反**：`1080 × 1920` px 畫布的安全區是中央 `1010 × 1280` px（`x = 35–1045`、`y = 220–1500`）。所有非全螢幕 motion graphic、B-ROLL、字幕、文字、Logo、貼圖、圖示、人物小窗等疊層，其完整可見範圍必須在安全區內。全螢幕背景可鋪滿，但重要文字、人臉、Logo 與視覺焦點仍須在安全區。建立 HyperFrames composition、預覽與 render 後抽幀時都要檢查；超出就修改，不得交付。
6. **剪完問一句**要不要把可重用元件（開場標題、數字動畫、終端機等）存進 `custom/`（見專案規則檔，如根目錄的 AGENTS.md / GEMINI.md 中的「剪完之後」說明）。
...
剪片的技術細節（Inventory、EDL、helpers、HyperFrames render 指令）一律照 `~/.codex/skills/video-use/SKILL.md` (或 `~/.gemini/config/skills/video-use/SKILL.md` / `~/.codex/skills/video-use/SKILL.md`) 與專案規則檔（例如根目錄的 AGENTS.md / GEMINI.md），不要在這裡重新發明。

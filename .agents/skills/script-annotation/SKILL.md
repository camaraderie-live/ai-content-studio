...
1. **先複述，再動手**（video-use SKILL.md Hard Rule 11 + 專案 AGENTS.md 第 6 點）：逐條講一次「這句話我會怎麼呈現」，把每一條的做法、缺的素材、模糊的地方講清楚，使用者說「對」才開始剪。標註常常很精簡，你以為的和他想的可能不一樣。
...
6. **剪完問一句**要不要把可重用元件（開場標題、數字動畫、終端機等）存進 `custom/`（見 AGENTS.md「剪完之後」）。
...
剪片的技術細節（Inventory、EDL、helpers、HyperFrames render 指令）一律照 `~/.codex/skills/video-use/SKILL.md` 與專案 AGENTS.md，不要在這裡重新發明。

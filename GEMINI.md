...
## Antigravity 開啟專案時：同步 Claude skills

每個新 Antigravity 工作階段開始時，先執行：

```bash
python3 .agents/skills/convert-claude-skills/scripts/convert.py --check
```

如果回報有 skill 尚未同步，立刻套用 `convert-claude-skills` skill 執行轉換，再繼續處理使用者需求。這個檢查只比較 `.claude/skills/` 與 `.agents/skills/`，不會更動檔案；Antigravity 專屬/共享的 skill 不會被刪除。
...
test -d ~/Developer/video-use || test -L ~/.gemini/config/skills/video-use   # video-use 引擎本體
[ -n "$ELEVENLABS_API_KEY" ] || grep -q '^ELEVENLABS_API_KEY=..' ~/.gemini/config/skills/video-use/.env 2>/dev/null   # 轉錄用的金鑰
...
- **缺 video-use 本體**：
  ...
  ln -sfn ~/Developer/video-use ~/.gemini/config/skills/video-use
  ```
...
### 檢查途中遇到任何問題：一定要提醒「不懂就直接問」

環境檢查只要出現 any 卡關、錯誤訊息，或你講到 any 專有名詞（Homebrew、ffmpeg、API 金鑰、終端機、npx、node……這種不寫程式的人不見得懂的東西），**每一次都要在訊息最後補一句**：「這裡如果有哪個字看不懂、或不確定要按什麼，直接在對話裡問我就好，不用自己 Google。」

不要覺得說過一次就夠了——只要又冒出新的名詞或新的錯誤，就再提醒一次。名詞第一次出現時，順手用一句白話解釋它是什麼。
...
- **ElevenLabs API 金鑰不存在**：告訴使用者你需要一把「轉錄用的鑰匙」，才能把影片裡講的話轉成文字。請他到 https://elevenlabs.io/app/settings/api-keys 註冊（有免費額度），建立具備 Speech to Text 權限的金鑰並貼給你。拿到後：
  ```bash
  printf 'ELEVENLABS_API_KEY=%s\n' "$KEY" > ~/.gemini/config/skills/video-use/.env
  chmod 600 ~/.gemini/config/skills/video-use/.env
  ```
...
2. **在這個專案資料夾裡，幫他建好兩個子資料夾**，把素材依用途分開放：
   - `A-ROLL/`：主影片、主聲道（口播、訪談這種「主線」的畫面 and 聲音）。
...
### 檢查完成後（續）：品牌檔案還是空的，就先問「你想怎麼開始」

環境檢查跑完後，**先看一眼 `我的品牌/` 資料夾**。如果它不存在、或裡面沒有 `我是誰.md` / `我怎麼說話.md`，代表這是全新的客戶——**不要等他開口，主動把所有能做的事列出來問他想從哪裡開始**，用白話講，不要用 skill 名稱：

> 「我這邊還沒有你的品牌檔案，等於還不認識你。你想從哪個開始都可以：
> 1. **貼 Reels 網址給我**——你自己的舊影片、或你欣賞的別人的影片都行，我幫你下載轉成文字稿，一支一支累積成你的素材庫（想貼幾支都可以）。
> 2. **讓我訪談你**——我一次問一題，把你是誰、在做什麼、講話的口氣整理成檔案。
> 3. **你直接講／貼給我**——自我介紹、過去的貼文、你想被記住的樣子，貼進來我幫你整理。
> 4. **先想題目**——不知道要發什麼？我幫你想選題。
> 5. **直接剪片**——手上已經有素材想剪，也可以先剪，品牌檔案之後再補。
>
> 想從哪個開始？（不確定的話我建議先 1 或 2，之後寫的每一篇才會像你講的話。）」

重點：**要讓客戶知道「貼 Reels 網址就能建檔」這件事存在**——這是最省力的入口，多數人不會自己想到。
選 1 走 `reels-transcribe`；選 2、3 走 `profile-library` 的訪談流程；選 4 走 `topic-generator`；選 5 直接進剪輯部五階段。
...
1. 讀取 `~/.gemini/config/skills/video-use/SKILL.md`，裡面的原則、Hard Rules、helpers 用法、流程（Inventory → 對話 → 提策略 → 執行 → 預覽 → 自我檢查 → 保存記憶）全部照做。
...
9. **盤點素材、或使用者搞不清楚主線該放什麼時，套用 `aroll-guide` skill** 幫他分清楚 A-ROLL（主線，可能是講話影片或旁白聲音） and B-ROLL（疊上去的素材），並判斷他屬於哪一種主線。特別是「沒有人講話、純畫面」那種，一定要確認多段影片有沒有先提取要用的片段、有沒有照順序用數字開頭命名。
...
1. **先複述**（同上面第 6 點、SKILL.md Hard Rule 11）：逐條講一次「這句話我會怎麼呈現」，把缺的素材（Antigravity 動畫、demo 影片、截圖等）列成清單請他丟進 `B-ROLL/`，確認「對」才動手。缺素材就照 [腳本動畫標註.md](./腳本動畫標註.md) 列的「你要準備的素材」欄提醒他，不要自己找替代品硬做。
...
影片渲染完、使用者滿意之後，**一定要主動問一句**：「這次做的東西裡，有沒有想存起來、下次直接套用的？最常見的是開場那段疊加文字，還有字幕的樣式——字體、大小、顏色、外框。」使用者說要存，就把那個元件複製一份到 `~/.gemini/config/skills/video-use/custom/<元件名>/`（例如 `custom/bilingual-captions/`、`custom/intro-title/`），一個元件一個資料夾，裡面放：

...
1. **先複述，再動手**（video-use SKILL.md Hard Rule 11 + 專案規則檔中的「複述確認」原則，例如根目錄的 CLAUDE.md / GEMINI.md）：逐條講一次「這句話我會怎麼呈現」，把每一條的做法、缺的素材、模糊的地方講清楚，使用者說「對」才開始剪。標註常常很精簡，你以為的和他想的可能不一樣。
...
6. **剪完問一句**要不要把可重用元件（開場標題、數字動畫、終端機等）存進 `custom/`（見專案規則檔，如根目錄的 CLAUDE.md / GEMINI.md 中的「剪完之後」說明）。
...
剪片的技術細節（Inventory、EDL、helpers、HyperFrames render 指令）一律照 `~/.claude/skills/video-use/SKILL.md` (或 `~/.gemini/config/skills/video-use/SKILL.md` / `~/.codex/skills/video-use/SKILL.md`) 與專案規則檔（例如根目錄的 CLAUDE.md / GEMINI.md），不要在這裡重新發明。
